        ...
        if (in_array($file_extension, $allowed)) {
            ...
            if ($file_size < 2000000) {
                ...
                */
                ...
                imagecopyresampled($thumb_img, $src_image, 0, 0, $src_x, $src_y, $thumb_size, $thumb_size, (int)$smaller_side, (int)$smaller_side);
                ...
                $validation_result = validate_unit_input($_POST['unit-name'], $_POST['race'], $_POST['unit-type'], $_POST['health'], $_POST['shield'], $_POST['armor'], $_POST['attack-damage'], $_POST['attack-range'], $_POST['movement-speed'], $_POST['mineral-cost'], $_POST['gas-cost'], $_POST['build-time'], $_POST['description'], $_POST['tags'], $_POST['cloaking'], $_POST['img-description'], $file_name_new, $unit_types, $races);
                ...
                if ($validation_result['is_valid']) {
                    ...
                    if (!$is_editing) {
                        if (insert_unit($data['name'], $data['race'], $data['unit_type'], $data['health'], $data['shield'], $data['armor'], $data['attack_damage'], $data['attack_range'], $data['movement_speed'], $data['mineral_cost'], $data['gas_cost'], $data['build_time'], $data['description'], $data['tags'], $data['has_cloak'], $data['img_description'], $data['file_name'])) {

                            $message = "Your entry was successfully added to our database.";
                            $alert_class = "alert-success";
                        } else {
                            $message = "There was a problem adding your unit: " . $connection->error;
                        }
                    } else {
                        if (update_unit($data['name'], $data['race'], $data['unit_type'], $data['health'], $data['shield'], $data['armor'], $data['attack_damage'], $data['attack_range'], $data['movement_speed'], $data['mineral_cost'], $data['gas_cost'], $data['build_time'], $data['description'], $data['tags'], $data['has_cloak'], $data['img_description'], $data['file_name'], $unit_id)) {
                            $message = "Your entry was successfully updated to our database.";
                            $alert_class = "alert-success";


                            $old_full_path = "images/full/" . $unit['filename'];
                            $old_thumb_path = "images/thumbs/" . $unit['filename'];

                            @unlink($old_full_path);
                            @unlink($old_thumb_path);
                        } else {
                            $message = "There was a problem updating your unit: " . $connection->error;
                        }
                    }
                } else {
...
//no photo editing
if (isset($_POST['submit']) && empty($_FILES['img-file']['name']) && $is_editing) {
    $validation_result = validate_unit_input($_POST['unit-name'], $_POST['race'], $_POST['unit-type'], $_POST['health'], $_POST['shield'], $_POST['armor'], $_POST['attack-damage'], $_POST['attack-range'], $_POST['movement-speed'], $_POST['mineral-cost'], $_POST['gas-cost'], $_POST['build-time'], $_POST['description'], $_POST['tags'], $_POST['cloaking'], $_POST['img-description'], $unit['filename'], $unit_types, $races);
    ...
    if ($validation_result['is_valid']) {
        ...
        if (update_unit($data['name'], $data['race'], $data['unit_type'], $data['health'], $data['shield'], $data['armor'], $data['attack_damage'], $data['attack_range'], $data['movement_speed'], $data['mineral_cost'], $data['gas_cost'], $data['build_time'], $data['description'], $data['tags'], $data['has_cloak'], $data['img_description'], $data['file_name'], $unit_id)) {
            $message = "Your entry {$data['name']} was successfully updated.";

            $alert_class = "alert-success";
        } else {
            $message = "There was error updating the unit.";
        }
    } else {
        // If the data is invalid, we'll implode our array of error messages. Because of the way things are printed in the alert box (and the fact that each error is just a string with no HTML syntax), we are separating each message with a closing and opening paragraph tag.
        $message = implode("</p><p>", $validation_result['errors']);
